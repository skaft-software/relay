...
import os, sys, subprocess, re, json, time, signal, shutil
...
def _resolve_llama_server():
    """Resolve llama-server path: env var → which() → hardcoded fallback."""
    env_path = os.environ.get('LLAMA_SERVER_PATH', None)
    if env_path and os.path.exists(env_path):
        return env_path
    which_path = shutil.which('llama-server')
    if which_path:
        return which_path
    return "/home/achu/llama.cpp/build-vulkan/bin/llama-server"
...
def generate_systemd_unit(mpath, ctx, ncm, cpm, draft_path, output_path,
    ...
    if llama_server_bin is None:
        llama_server_bin = _resolve_llama_server()

    user = os.environ.get('USER', 'achu')
    llama_cpp_dir = os.environ.get('LLAMA_CPP_DIR', '/home/achu/llama.cpp')
    ...
    lines.append(f"WorkingDirectory={llama_cpp_dir}")
