...
export type AppConfig = {
  ...
  /** Switch policy for model changes. Only eager is supported. */
  switchPolicy?: 'eager';
...
function readSwitchPolicy(value: string | undefined): 'eager' {
  const raw = readOptional(value) ?? 'eager';
  if (raw === 'eager') return 'eager';
  throw new Error('RELAY_SWITCH_POLICY=graceful is no longer supported; use eager');
}
